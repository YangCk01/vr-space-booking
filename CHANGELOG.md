...
- **编辑保存失败**：后端 `createStaff` / `updateStaff` 的 `validRoles` 缺少 `ADMIN`，导致编辑/创建「管理员」角色员工时返回 400 "无效的角色"
  - 修复：`validRoles` 新增 `'ADMIN'`
- **前端角色下拉**：新增「管理员」选项，解决编辑 ADMIN 员工时角色显示异常
- **主账号编辑保护**：编辑 `SUPER_ADMIN` 时前端禁用保存按钮并显示提示「主账号不可编辑」，避免后端返回 403 但页面无反馈
- **代码规范**：`StaffFormSheet` 中将 `useMemo` 副作用误用改为 `useEffect`
...
- **订单模型扩展**：`Order` 新增 `expireAt` 字段，创建订单时自动设置 **30 分钟支付倒计时**
- **C 端支付页 (`Pay.tsx`)**：
  - 实时倒计时显示（分:秒），过期后禁止支付
  - 支持余额支付，自动检查本金+赠金余额
  - 新增取消订单入口与二次确认弹窗
  - 支付成功跳转 `/success` 时携带完整预约信息
- **C 端订单列表 (`Orders.tsx`)**：
  - `PENDING` 订单显示倒计时与过期状态
  - 根据开场时间计算**阶梯退费规则**并展示最迟取消时间
- **定时任务 (`orderTimeoutJob.ts`)**：
  - 每分钟扫描过期 `PENDING` 订单
  - 自动取消订单、恢复关联排场、返还已预占优惠券
...
- 创建订单时**不再预占**优惠券
- 支付成功后才会扣减优惠券状态为 `USED`
- 过期取消时自动恢复优惠券为 `UNUSED`

---

### 🔐 C 端账户安全与资料

- **账户设置页 (`AccountSettings.tsx`) 重构**：
  - 新增**生日**字段（`birthday`），支持日期选择
  - 优化头像上传、错误提示、成功通知体验
  - 移除页内修改密码/手机号逻辑，改为独立页面入口
- **新增修改手机号页面** (`ChangePhonePage.tsx`)
- **新增修改密码页面** (`ChangePasswordPage.tsx`)
- `App.tsx` 新增 `/change-phone` 与 `/change-password` 路由

---

### 💰 财务体验优化

- **财务页面文案升级**：将专业会计术语改为更直观的运营语言
  - "每日现金解缴表（收付实现制）" → "每日现金收支表（按实际收付）"
  - "每日确权营收表（权责发生制）" → "每日营收确认表（按服务完成）"
  - "在线直付" → "在线支付金额/营收"
  - "会员本金确权" → "余额消费营收"
- **用户详情弹窗**：
  - "赠送记录" Tab 升级为 **"财务记录"**
  - 左侧展示积分赠送、优惠券赠送记录
  - 右侧新增**充值记录**列表（调用 `getUserRechargeRecords`）

---

### 🔧 权限与系统清理

- **后端权限种子 (`seedPermissions.ts`)**：
  - 自动清理旧格式菜单权限（`home`、`venues`、`orders` 等 11 个 legacy key）
- **前端 AuthGuard**：
  - 移除旧格式权限检测逻辑，避免重复登出
  - 认证失败时统一调用 `logout()` 清理状态
- **TopBar 全局刷新**：
  - 由逐个 `invalidateQueries` 改为统一 `queryClient.invalidateQueries()`

---

### 🐛 其他修复

- 新增 `fixReconciliation.ts` 对账修复脚本
- `PointsOrder` 模型新增 `venueId` 字段，支持积分商品关联场地
- `PointsMall`、`campaignController`、`rechargeController` 等细节修复与联调
...
| 文件 | 说明 |
|------|------|
| `app/reservation/src/pages/ChangePhonePage.tsx` | C 端修改手机号页面 |
| `app/reservation/src/pages/ChangePasswordPage.tsx` | C 端修改密码页面 |
| `server/src/jobs/orderTimeoutJob.ts` | 订单超时自动取消定时任务 |
| `server/scripts/fixReconciliation.ts` | 对账差异修复脚本 |
