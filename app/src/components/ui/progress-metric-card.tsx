...
export default function ProgressMetricCard({
  ...
  // Les chiffres agrègent toutes les séries (total par jour) pour refléter
  // le vrai total affiché (ex. revenus complets). Les props restent prioritaires.
  const stats = useMemo(() => {
    const dates = primary?.data.map((d) => d.date) ?? [];
    const totals = dates.map((date, i) =>
      visibleSeries.reduce((acc, s) => acc + (s.data[i]?.value ?? 0), 0),
    );
    const sum = totals.reduce((a, b) => a + b, 0);
    const first = totals[0] ?? 0;
    const last = totals[totals.length - 1] ?? 0;
    const prev = totals[totals.length - 2] ?? first;
      ...
      <div className={shell}>
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-1 py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground">数据可用后将显示指标。</p>
          </div>
        </div>
      </div>
    );
      ...
      >
        ...
        {showStats && (
          <div className="flex items-center gap-2.5 text-[12px] text-muted-foreground">
            <span>
              <span className="font-medium text-foreground/80">{fmtCompact(stats.peak)}</span> {peakLabel}
            </span>
            <span className="opacity-40">·</span>
            <span>
              <span className="font-medium text-foreground/80">{fmtCompact(stats.low)}</span> {lowLabel}
            </span>
            <span className="opacity-40">·</span>
            <span>
