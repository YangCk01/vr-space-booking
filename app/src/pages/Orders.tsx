...
> = {
  ...
  no_show: {
    bg: "bg-vrtext-muted/15",
    text: "text-vrtext-muted",
...
const orderTypeLabel = (order: Order) => {
  if (order.orderKind === "FEE") return "改签费";
  if (order.groupBuyPackageId) return "团购订单";
  return "预约订单";
};

const orderGameTitle = (order: Order) => {
  if (order.orderKind === "FEE") {
    return order.parentOrder?.booking?.game?.title || "VR体验";
  }
  return order.groupBuyPackage?.title || order.booking?.game?.title || "VR体验";
};

const yuan = (value?: number | null) => Number(((value || 0) / 100).toFixed(2));
...
function StatusBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full px-3 py-1 text-vr-caption font-medium whitespace-nowrap",
        cfg.bg,
        cfg.text,
      )}
    >
...
function AssignedEquipmentCard({
  bookingId,
  status,
}: {
  bookingId: string;
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient
        .get(`/bookings/${bookingId}/equipment`)
        .then((r) => r.data.data),
    enabled: !!bookingId,
  });
  ...
  return (
    <div className="bg-vrbg-elevated rounded-lg p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="text-vr-body-sm text-vrtext-secondary font-medium">
          已分配设备
        </h4>
        <span
          className={cn(
            ...
            isReleased
              ? "bg-vrtext-muted/10 text-vrtext-muted"
              : "bg-vraccent-primary/10 text-vraccent-primary",
          )}
        >
      ...
      {devices.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {devices.map((d: any) => (
            <div
              key={d.id}
              className="flex items-center justify-between text-vr-body-sm"
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-vr-caption text-vrtext-muted font-mono">
                  {d.code}
                </span>
              </div>
...
function OrderDetailSheet({
  ...
  onGroupRedeem: (code: string) => void;
  canHandleNoShowRefund: boolean;
  canRefund: boolean;
  canVerify: boolean;
  canReschedule: boolean;
  hasPendingNoShowApproval: boolean;
  ...
  const relatedBooking =
    order.orderKind === "FEE" ? order.parentOrder?.booking : order.booking;
  const timeline = [
    {
      ...
      time:
        statusLower === "pending"
          ...
          : statusLower !== "cancelled"
            ? formatDateTime(order.createdAt)
            : "",
      status: statusLower === "cancelled" ? "已取消" : "未付款",
      desc: statusLower === "cancelled" ? "订单已取消" : "等待用户付款",
    ...
    {
      time: ["completed", "no_show"].includes(statusLower)
        ? order.bookingTime?.split(" ")[0] || "-"
        : "",
      status: statusLower === "no_show" ? "已作废" : "已核销",
      desc: statusLower === "no_show" ? "顾客超时未到场" : "体验已核销",
      completed: ["completed", "no_show"].includes(statusLower),
    },
  ];
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-[480px] bg-vrbg-card border-l border-vrborder-subtle p-0 sm:max-w-[480px]"
      >
        <SheetHeader className="p-6 border-b border-vrborder-subtle">
          <div className="flex items-center justify-between">
            ...
            <StatusBadge
              status={statusLower}
              statusText={statusLabelMap[order.status] || order.status}
            />
          </div>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  >
                    ...
                    {idx < timeline.length - 1 && (
                      <div
                        className={cn(
                          ...
                          item.completed && timeline[idx + 1].completed
                            ? "bg-vraccent-primary"
                            : "bg-vrbg-elevated",
                        )}
                      />
                    ...
                    <div
                      className={cn(
                        ...
                        item.completed
                          ? "bg-vraccent-primary border-vraccent-primary"
                          : "bg-transparent border-vrborder-subtle",
                      )}
                    />
              ...
              {[
                {
                  ...
                  icon: <MapPin className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "预约时间",
                  value: order.bookingTime,
                  icon: <Calendar className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "游戏",
                  value: relatedBooking?.game?.title || "VR体验",
                  icon: <Gamepad2 className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "预约类型",
                  value: order.orderKind === "FEE" ? "改签" : "散客预约",
                  icon: <User className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "预约人数",
                  value: relatedBooking?.personCount ?? "-",
                  icon: <User className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "预约人",
                  value: relatedBooking?.personName || "-",
                  icon: <User className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "联系电话",
                  value: relatedBooking?.personPhone || "-",
                  icon: <Phone className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "订单金额",
                  value: "",
                ...
                {
                  label: "实付金额",
                  value: `¥${((order.amount || 0) / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                  icon: <Receipt className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "支付方式",
                  value:
                    payMethodLabelMap[order.payMethod || ""] ||
                    order.payMethod ||
                    "-",
                  icon: <PaymentIcon className="w-4 h-4 text-vrtext-muted" />,
                },
                {
                  label: "创建时间",
                  value: formatDateTime(order.createdAt),
                  icon: <Clock className="w-4 h-4 text-vrtext-muted" />,
                },
              ].map((item, idx) => (
                <div key={idx} className="flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      ...
                      item.label === "订单金额" &&
                        "font-semibold text-vrwarning",
                    )}
                  >
                    {item.label === "订单金额"
                      ? `¥${((order.originalAmount || order.amount || 0) / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                      : item.value}
                  </span>
                </div>
              ))}
          ...
          {/* 改签费订单：关联原消费订单 */}
          {order.orderKind === "FEE" && order.parentOrder && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.35, duration: 0.3 }}
              className="bg-vrbg-elevated rounded-xl p-5 space-y-3"
            >
              <h4 className="text-vr-body-sm text-vrtext-secondary font-medium">
                关联原消费订单
              </h4>
              <button
                onClick={() => {
                  onOpenChange(false);
                  navigate(
                    `/orders?search=${encodeURIComponent(order.parentOrder!.orderNo)}`,
                  );
                }}
                className="w-full flex items-center justify-between p-3 rounded-lg bg-vrbg-surface hover:bg-vrbg-elevated border border-vrborder-subtle transition-colors"
              >
                <div className="flex flex-col items-start">
                  <span className="text-vr-body-sm text-vraccent-primary font-mono">
                    {order.parentOrder.orderNo}
                  </span>
                  <span className="text-vr-caption text-vrtext-muted">
                    {order.feeReason || "改签手续费"}
                  </span>
                </div>
            ...
            order.feeOrders.length > 0 && (
              ...
              >
                ...
                <div className="space-y-2">
                  {order.feeOrders.map((feeOrder) => (
                    <button
                      key={feeOrder.id}
                      onClick={() => {
                        onOpenChange(false);
                        navigate(
                          `/orders?search=${encodeURIComponent(feeOrder.orderNo)}`,
                        );
                      }}
                      className="w-full flex items-center justify-between p-3 rounded-lg bg-vrbg-surface hover:bg-vrbg-elevated border border-vrborder-subtle transition-colors"
                    >
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-vr-body-sm text-vrerror font-semibold">
                    -¥{((order.refundAmount || 0) / 100).toFixed(2)}
                  </span>
                </div>
                {order.cancelledAt && (
                  <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-center py-2">
            <span
              className={cn(
                "inline-flex items-center gap-2 rounded-full px-4 py-2 text-vr-body-sm font-medium",
                cfg?.bg,
                cfg?.text,
              )}
            >
          ...
          {latestApproval && (
            <motion.div
              ...
              className={cn(
                ...
                approvalStatusClass[latestApproval.status] ||
                  "border-vrborder-subtle bg-vrbg-elevated text-vrtext-secondary",
              )}
            >
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-vr-caption opacity-80 mt-0.5">
                    {approvalActionText(latestApproval)} · ¥
                    {((latestApproval.amount || 0) / 100).toFixed(2)}
                  </p>
                </div>
                <span className="rounded-full bg-black/10 px-2.5 py-1 text-vr-caption font-medium">
                  {approvalStatusLabel[latestApproval.status] ||
              ...
              {latestApproval.status === "EXECUTION_FAILED" &&
                latestApproval.approvalComment && (
                  <div className="rounded-lg bg-black/10 p-3 text-vr-caption leading-5">
        ...
        {/* Bottom Actions（改签费订单不展示操作按钮） */}
        {order.orderKind !== "FEE" && (
          <div className="p-6 border-t border-vrborder-subtle flex flex-wrap gap-3">
            {statusLower === "pending" && (
              <>
                <button
            ...
            {["paid", "ready_to_verify"].includes(statusLower) && (
              <>
                {canVerify && statusLower === "paid" &&
                  order.groupBuyPackageId &&
                  !order.bookingId && (
                    <button
                      onClick={() =>
                        onGroupRedeem(order.verifyCode || order.orderNo)
                      }
                      className="w-full h-10 rounded-lg bg-vrsuccess text-white text-vr-body-sm font-medium hover:bg-vrsuccess/90 transition-colors"
                    >
                      团购核销
                    </button>
                  )}
            ...
            {["completed", "no_show"].includes(statusLower) && (
              <>
                ...
                <button className="flex-1 h-10 rounded-lg bg-vrbg-elevated text-vrtext-secondary text-vr-body-sm font-medium cursor-default whitespace-nowrap">
                  {statusLower === "no_show" ? "已作废" : "已核销"}
                </button>
              </>
            )}
            {canRefund && statusLower === "refunding" && (
              <button
...
export default function Orders() {
  ...
  const [orderTypeFilter, setOrderTypeFilter] = useState<
    "all" | "RESERVATION" | "GROUP_BUY" | "FEE"
  >("all");
  const [orderKindFilter, setOrderKindFilter] = useState<
    "all" | "NORMAL" | "FEE"
  >("all");
  const [refundStatusFilter, setRefundStatusFilter] = useState<
    "all" | "HAS_REFUND" | "NO_REFUND"
  >("all");
  const [parentOrderNoFilter, setParentOrderNoFilter] = useState("");
  ...
  const [verifyTargetOrder, setVerifyTargetOrder] = useState<Order | null>(
    null,
  );
  const [skipScanVerify, setSkipScanVerify] = useState(false);
  ...
  // 团购券核销抽屉状态
  const [groupRedeemOpen, setGroupRedeemOpen] = useState(false);
  const [groupRedeemCode, setGroupRedeemCode] = useState("");
  ...
  const [refundAction, setRefundAction] =
    useState<RefundDispositionAction>("FULL_REFUND");
  ...
  const { data: orderData, isFetching } = useQuery({
    ...
    queryFn: () =>
      getOrders({
        ...
        orderKind: orderKindFilter === "all" ? undefined : orderKindFilter,
        refundStatus:
          refundStatusFilter === "all" ? undefined : refundStatusFilter,
        parentOrderNo: parentOrderNoFilter || undefined,
        page: currentPage,
        pageSize,
      }),
  ...
  const pendingNoShowApprovalMap = useMemo(() => {
    ...
    return new Set<string>(
      items
        .filter(
          (item: any) =>
            item.status === "PENDING" && item.type === "NO_SHOW_REFUND",
        )
  ...
  const pendingRefundApprovalMap = useMemo(() => {
    ...
    return new Set<string>(
      items
        .filter(
          (item: any) =>
            item.status === "PENDING" && item.type === "ORDER_REFUND",
        )
        .map((item: any) => item.targetId),
    );
  }, [approvalData]);
  ...
  const canVerifyOrders = hasPermission(currentUser, "order:verify");
  const canRefundOrders = hasPermission(currentUser, "order:refund");
  ...
  const openRefundDialog = (order: Order) => {
    const defaultRefund = Math.max(
      0,
      (order.amount || 0) - (order.penaltyAmount || 0),
    );
    ...
    setRefundAmountYuan(
      (
        (order.status === "NO_SHOW" ? defaultRefund : order.amount || 0) / 100
      ).toFixed(2),
    );
    setRefundReason("");
    setRefundOpen(true);
  ...
  const handleScanVerify = async (code: string) => {
    ...
    try {
      ...
      if (order.groupBuyPackageId) {
        setGroupRedeemCode(trimmed);
        setGroupRedeemOpen(true);
        return;
      }
      if (!["PAID", "READY_TO_VERIFY"].includes(order.status)) {
        alert(
          `该订单当前状态为「${statusLabelMap[order.status] || order.status}」，无法核销`,
        );
        return;
      }
      setVerifyTargetOrder(order);
      setSkipScanVerify(true);
  ...
  const rescheduleMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiClient.post(`/bookings/${orderId}/reschedule`, {
        date,
        startTime,
      });
      return res.data;
    ...
    onSuccess: (data: any) => {
      ...
      if (delta > 0) msg += `需补差价：¥${(delta / 100).toFixed(2)}。`;
        ...
        msg += `退回差价：¥${(Math.abs(delta) / 100).toFixed(2)}。`;
      alert(msg);
    },
    onError: (error: any) => {
      alert(
  ...
  const handleExport = async () => {
    ...
    const rows = allOrders.map((o) => ({
      订单号: o.orderNo,
      订单类型: orderTypeLabel(o),
      关联订单: o.parentOrder?.orderNo || "-",
      用户: o.user?.name || o.booking?.personName || o.customer || "-",
      用户手机: o.user?.phone || o.booking?.personPhone || o.phone || "-",
      场地: o.venueName || "-",
      创建时间: o.createdAt
        ? format(new Date(o.createdAt), "yyyy-MM-dd HH:mm:ss")
        : "-",
      预约时间: o.bookingTime || "-",
      游戏或套餐: orderGameTitle(o),
      人数: o.orderKind === "FEE" ? "-" : o.booking?.personCount ?? "-",
      原价: yuan(o.originalAmount || o.amount),
      会员优惠: yuan(o.discountAmount),
      优惠券抵扣: yuan(o.couponDiscount),
      总优惠: yuan((o.discountAmount || 0) + (o.couponDiscount || 0)),
      实付: yuan(o.amount),
      退款金额: yuan(o.refundAmount),
      订单状态: statusLabelMap[o.status] || o.status,
      支付方式: payMethodLabelMap[o.payMethod || ""] || o.payMethod || "-",
      费用类型: o.orderKind === "FEE" ? o.feeType || "改签费" : "消费",
      费用说明: o.feeReason || "-",
      核销码: o.verifyCode || "-",
    }));
  ...
  const selectedOrders = useMemo(
    () => paginatedOrders.filter((o) => selectedIds.includes(o.id)),
    [paginatedOrders, selectedIds],
  );
  const hasPaidSelected = selectedOrders.some((o) => o.status === "PAID");
  const hasReadyToVerifySelected = selectedOrders.some(
    (o) => o.status === "READY_TO_VERIFY",
  );
  const hasPendingSelected = selectedOrders.some((o) => o.status === "PENDING");

  // 标签计数：状态 tab 只统计消费订单（NORMAL），避免改签费订单混入角标；全部 tab 显示当前筛选总数（不受 status 过滤影响）
  const tabCounts = useMemo(() => {
    const all =
      (orderData?.meta?.totalAll as number | undefined) ??
      (orderData?.meta?.total as number | undefined) ??
      total;
    // 当筛选消费订单时，状态角标仅统计消费订单；其余情况统计全部订单（含改签费）
    const backendCounts = (
      orderKindFilter === "NORMAL"
        ? orderData?.meta?.normalStatusCounts
        : orderData?.meta?.statusCounts
    ...
    for (const o of apiOrders) {
      if (orderKindFilter !== "FEE" && o.orderKind === "FEE") continue;
      const key = o.status.toLowerCase();
      counts[key] = (counts[key] || 0) + 1;
    }
    return counts;
  }, [orderData?.meta, orderKindFilter, apiOrders, total]);
    ...
    <Layout breadcrumb={["订单管理"]}>
      ...
      >
        ...
        <div className="flex items-end justify-between">
          ...
          >
            <h1 className="text-vr-h1 text-vrtext-primary font-semibold">
              订单管理
            </h1>
          ...
          <span className="text-vr-caption text-vrtext-tertiary">
            共 <span className="text-vrtext-primary font-medium">{total}</span>{" "}
            条记录
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <motion.div
            ...
            className="relative"
          >
            <Receipt className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-vrtext-muted" />
            <input
              type="text"
              placeholder="关联订单号"
              value={parentOrderNoFilter}
              onChange={(e) => {
                setParentOrderNoFilter(e.target.value);
                setCurrentPage(1);
              }}
              className="w-[160px] h-9 pl-9 pr-3 bg-vrbg-surface border border-vrborder-subtle rounded-lg text-vr-body-sm text-vrtext-primary placeholder:text-vrtext-muted focus:outline-none focus:border-vraccent-primary focus:ring-1 focus:ring-vraccent-primary/15 transition-all"
            />
          </motion.div>
          ...
          {canVerifyOrders && (
            <motion.button
              ...
              className="h-9 px-4 rounded-lg bg-vrsuccess text-white text-vr-body-sm font-medium hover:bg-vrsuccess/90 transition-colors disabled:opacity-50 flex items-center gap-2"
            >
              <ScanLine className="w-4 h-4" />
              {scanLoading ? "识别中..." : "扫码验券"}
            </motion.button>
          )}
        </div>

        {/* Status Tabs */}
        <div className="border-b border-vrborder-subtle">
          <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide py-1">
            <span className="shrink-0 text-vr-body-sm font-semibold text-vrtext-primary">
              订单状态:
            </span>
            {tabs.map((tab, idx) => (
              <motion.button
                ...
                className={cn(
                  "relative shrink-0 px-4 py-1.5 rounded-full text-vr-body-sm font-medium transition-colors",
                  activeTab === tab.key
                    ? "bg-vraccent-primary text-white"
                    : "bg-vrbg-elevated text-vrtext-secondary hover:text-vrtext-primary hover:bg-vrbg-surface",
                )}
              >
                      ...
                      <span
                        className={cn(
                          ...
                          activeTab === tab.key
                            ...
                            : "bg-vrbg-surface text-vrtext-muted",
                        )}
                      >
                        {tabCounts[tab.key]}
                      </span>
            ...
            <div className="flex items-center gap-1 bg-vrbg-surface rounded-lg p-1">
              {(
                ...
                <button
                  key={s.key}
                  onClick={() => {
                    setOrderKindFilter(s.key);
                    setCurrentPage(1);
                  }}
                  className={cn(
                    "px-3 py-1 rounded text-vr-body-sm font-medium transition-colors",
                    orderKindFilter === s.key
                      ? "bg-vraccent-primary text-white"
                      : "text-vrtext-secondary hover:text-vrtext-primary",
                  )}
                >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-vrbg-surface rounded-lg p-1">
              {(
                [
                  { key: "all", label: "全部" },
                  { key: "HAS_REFUND", label: "有退款" },
                  { key: "NO_REFUND", label: "无退款" },
                ] as const
              ).map((s) => (
                <button
                  key={s.key}
                  onClick={() => {
                    setRefundStatusFilter(s.key);
                    setCurrentPage(1);
                  }}
                  className={cn(
                    "px-3 py-1 rounded text-vr-body-sm font-medium transition-colors",
                    refundStatusFilter === s.key
                      ? "bg-vraccent-primary text-white"
                      : "text-vrtext-secondary hover:text-vrtext-primary",
                  )}
                >
          ...
          {selectedIds.length > 0 && (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                {hasReadyToVerifySelected && canVerifyOrders && (
                  <button
                    onClick={() => {
                      const ids = selectedOrders
                        .filter((o) => o.status === "READY_TO_VERIFY")
                        .map((o) => o.id);
                      if (
                        !window.confirm(
                          `确定要批量核销 ${ids.length} 个订单吗？`,
                        )
                      )
                        return;
                ...
                {hasPaidSelected && canRefundOrders && (
                  <button
                    onClick={() => {
                      const ids = selectedOrders
            ...
            <table className="w-full table-fixed">
              <thead>
                <tr className="bg-vrbg-elevated">
                  <th className="text-center px-1.5 py-3 text-vr-caption text-vrtext-secondary font-medium w-[38px]">
                    <input
                      type="checkbox"
                      checked={
                      ...
                      onChange={() => {
                        const allSelected = paginatedOrders.every((o) =>
                          selectedIds.includes(o.id),
                        );
                        if (allSelected) {
                          setSelectedIds((prev) =>
                            prev.filter(
                              (id) => !paginatedOrders.some((o) => o.id === id),
                            ),
                          );
                        } else {
                          setSelectedIds((prev) => [
                  ...
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[150px] whitespace-nowrap">
                    订单号
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[88px] whitespace-nowrap">
                    订单类型
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[120px] whitespace-nowrap">
                    关联订单
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[92px] whitespace-nowrap">
                    用户
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[70px] whitespace-nowrap">
                    场地
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[90px] whitespace-nowrap">
                    创建时间
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[190px] whitespace-nowrap">
                    预约时间
                  </th>
                  <th className="text-left px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[260px] whitespace-nowrap">
                    游戏
                  </th>
                  <th className="text-center px-1.5 py-3 text-vr-caption text-vrtext-secondary font-medium w-[48px] whitespace-nowrap">
                    人数
                  </th>
                  <th className="text-right px-1.5 py-3 text-vr-caption text-vrtext-secondary font-medium w-[78px] whitespace-nowrap">
                    实付
                  </th>
                  <th className="text-right px-1.5 py-3 text-vr-caption text-vrtext-secondary font-medium w-[72px] whitespace-nowrap">
                    优惠
                  </th>
                  <th className="text-right px-1.5 py-3 text-vr-caption text-vrtext-secondary font-medium w-[76px] whitespace-nowrap">
                    退款
                  </th>
                  <th className="text-center px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[86px] whitespace-nowrap">
                    状态
                  </th>
                  <th className="text-right px-2 py-3 text-vr-caption text-vrtext-secondary font-medium w-[50px] whitespace-nowrap">
                    操作
                  </th>
                </tr>
              </thead>
                  ...
                  {paginatedOrders.map((order, idx) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-3">
                        <span className="block truncate text-vr-body-sm text-vrtext-primary font-mono" title={order.orderNo}>
                          {order.orderNo}
                        </span>
                      </td>
                      <td className="px-2 py-3">
                        {order.orderKind === "FEE" ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-vr-caption font-medium bg-vrwarning/10 text-vrwarning">
                            改签费
                          </span>
                        ) : order.groupBuyPackageId ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-vr-caption font-medium bg-vraccent-primary/10 text-vraccent-primary">
                            团购
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-vr-caption font-medium bg-vrbg-elevated text-vrtext-secondary">
                            预约
                          </span>
                        )}
                      </td>
                      <td className="px-2 py-3">
                        {order.parentOrder ? (
                          <button
                            ...
                            className="block max-w-full truncate text-vr-body-sm text-vraccent-primary hover:underline font-mono"
                            title={order.parentOrder.orderNo}
                          >
                            {order.parentOrder.orderNo}
                          </button>
                        ) : (
                      ...
                      <td className="px-2 py-3">
                        <span className="block truncate text-vr-body-sm text-vrtext-primary" title={order.user?.name || order.booking?.personName || "-"}>
                          {order.user?.name || order.booking?.personName || "-"}
                        </span>
                      </td>
                      <td className="px-2 py-3">
                        <span className="block truncate text-vr-body-sm text-vrtext-primary" title={order.venueName}>
                          {order.venueName}
                        </span>
                      </td>
                      <td className="px-2 py-3">
                        <span className="block truncate text-vr-body-sm text-vrtext-primary" title={formatDateTime(order.createdAt)}>
                          {formatDateTime(order.createdAt)}
                        </span>
                      </td>
                      <td className="px-2 py-3">
                        <span
                          className="block text-vr-body-sm text-vrtext-primary whitespace-nowrap"
                          title={order.bookingTime}
                        >
                          {order.bookingTime}
                        </span>
                      </td>
                      <td className="px-2 py-3">
                        <span className="block truncate text-vr-body-sm text-vrtext-primary" title={
                          order.orderKind === "FEE"
                            ? order.parentOrder?.booking?.game?.title || "VR体验"
                            : order.groupBuyPackage?.title ||
                              order.booking?.game?.title ||
                              "VR体验"
                        }>
                          {order.orderKind === "FEE"
                            ? order.parentOrder?.booking?.game?.title ||
                              "VR体验"
                            : order.groupBuyPackage?.title ||
                              order.booking?.game?.title ||
                              "VR体验"}
                      ...
                      <td className="px-1.5 py-3 text-center">
                        <span className="text-vr-body-sm text-vrtext-primary">
                          {order.booking?.personCount ?? "-"}
                        </span>
                      </td>
                      <td className="px-1.5 py-3 text-right">
                        <span className="text-vr-body-sm text-vrtext-primary font-semibold whitespace-nowrap">
                          ¥{((order.amount || 0) / 100).toLocaleString(undefined, {
                      ...
                      <td className="px-1.5 py-3 text-right">
                        {(order.couponDiscount || 0) + (order.discountAmount || 0) > 0 ? (
                          <span className="text-vr-caption text-vrsuccess whitespace-nowrap">
                      ...
                      <td className="px-1.5 py-3 text-right">
                        {order.refundAmount && order.refundAmount > 0 ? (
                          <span className="text-vr-caption text-vrerror whitespace-nowrap">
                            -¥{((order.refundAmount || 0) / 100).toFixed(2)}
                          </span>
                        ) : (
                          <span className="text-vr-caption text-vrtext-muted">-</span>
      ...
      <OrderDetailSheet
        ...
        onReschedule={(o) => {
          ...
          setRescheduleDate(o.bookingTime?.split(" ")[0] || "");
          setRescheduleTime(o.bookingTime?.split(" ")[1]?.split("-")[0] || "");
          setRescheduleOpen(true);
        }}
        onGroupRedeem={(code) => {
          setDrawerOpen(false);
          setGroupRedeemCode(code);
          setGroupRedeemOpen(true);
        }}
        canHandleNoShowRefund={canHandleNoShowRefund}
        canRefund={canRefundOrders}
        canVerify={canVerifyOrders}
        canReschedule={canRescheduleOrders}
        hasPendingNoShowApproval={
          selectedOrder ? pendingNoShowApprovalMap.has(selectedOrder.id) : false
        }
        hasPendingRefundApproval={
          selectedOrder ? pendingRefundApprovalMap.has(selectedOrder.id) : false
        }
        latestApproval={
          selectedOrder ? latestApprovalMap.get(selectedOrder.id) : null
        }
      ...
      <PaymentMethodModal
        ...
        customer={
          paymentTargetOrder?.customer ||
          paymentTargetOrder?.user?.name ||
        ...
        order={
          verifyTargetOrder
            ? {
                ...
                customer:
                  verifyTargetOrder.customer ||
                  verifyTargetOrder.user?.name ||
                  verifyTargetOrder.booking?.personName,
      ...
      {/* 团购券核销抽屉 */}
      <GroupRedeemModal
        open={groupRedeemOpen}
        onClose={() => {
          setGroupRedeemOpen(false);
          setGroupRedeemCode("");
        }}
        code={groupRedeemCode}
        onSuccess={() => {
          invalidateAll();
          setDrawerOpen(false);
        }}
        ...
        {restoreOpen && restoreTarget && (
          ...
          >
            ...
            >
              ...
              <div className="mt-4">
                <label className="text-vr-caption text-vrtext-secondary block mb-1">
                  恢复原因
                </label>
                <textarea
                  value={restoreReason}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div className="rounded-xl bg-vrbg-surface border border-vrborder-subtle p-3">
                        ...
                        <p className="text-vr-body font-semibold text-vrtext-primary mt-1">
                      ...
                      <div className="rounded-xl bg-vrbg-surface border border-vrborder-subtle p-3">
                        ...
                        <p className="text-vr-body font-semibold text-vrwarning mt-1">
                      ...
                      <div className="rounded-xl bg-vrbg-surface border border-vrborder-subtle p-3">
                        ...
                        <p className="text-vr-body font-semibold text-vrsuccess mt-1">
                          ¥{(suggestedRefund / 100).toFixed(2)}
                        </p>
                      </div>
                    </div>
                    ...
                    {isNoShow && (
                      <div className="space-y-2 mb-4">
                        {[
                          {
                            key: "NO_REFUND" as const,
                            title: "不退款",
                            desc: "维持作废结果，仅记录处置原因。",
                          },
                          {
                            key: "PARTIAL_REFUND" as const,
                            title: "部分退款",
                            desc: "扣除违约金后退还剩余金额，可手动调整。",
                          },
                          {
                            key: "FULL_REFUND" as const,
                            title: "全额退款",
                            desc: "用于门店原因、设备故障或误操作等特殊情况。",
                          },
                        ].map((item) => (
                          <button
                            key={item.key}
                            ...
                            onClick={() => {
                              ...
                              if (item.key === "NO_REFUND")
                                setRefundAmountYuan("0.00");
                              if (item.key === "PARTIAL_REFUND")
                                setRefundAmountYuan(
                                  (suggestedRefund / 100).toFixed(2),
                                );
                              if (item.key === "FULL_REFUND")
                                setRefundAmountYuan((amount / 100).toFixed(2));
                            }}
                            className={cn(
                          ...
                          >
                            <div className="flex items-center justify-between gap-3">
                              <span className="text-vr-body-sm font-semibold text-vrtext-primary">
                                {item.title}
                              </span>
                              <span
                                className={cn(
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-vrtext-muted text-vr-body-sm">
                            ¥
                          </span>
                          <input
                            type="number"
                            ...
                            onChange={(e) =>
                              setRefundAmountYuan(e.target.value)
                            }
                            disabled={
