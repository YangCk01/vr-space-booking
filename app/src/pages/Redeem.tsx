...
import { Search, ScanLine, Ticket, Calendar, Clock, User, Phone, Users, MapPin, CheckCircle2, Loader2 } from 'lucide-react'
...
export default function Redeem() {
  ...
  const { data: orderRes, isLoading: orderLoading, error: orderError } = useQuery({
    queryKey: ['redeem-order', searchCode],
  ...
  useEffect(() => {
    if (order && venues.length > 0 && initRef.current !== order.id) {
      ...
      const bookingDate = booking?.date ? (typeof booking.date === 'string' ? booking.date.slice(0, 10) : new Date(booking.date).toISOString().slice(0, 10)) : ''
  ...
  return (
    <Layout breadcrumb={['团购券核销']}>
      <div className="max-w-4xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-vrbg-card border border-vrborder-subtle rounded-2xl p-6 shadow-sm"
        >
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-lg font-bold text-vrtext-primary">团购券核销</h1>
              <p className="text-xs text-vrtext-secondary">输入券码查询团购券订单并绑定预约</p>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-6">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="请输入团购券码或订单号"
                className="w-full h-10 pl-9 pr-10 rounded-lg bg-vrbg-surface border border-vrborder-subtle text-vr-body-sm text-vrtext-primary outline-none focus:border-vraccent-primary"
              />
              <button
                type="button"
                onClick={() => setScanOpen(true)}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-md bg-vrbg-elevated text-vrtext-muted hover:text-vraccent-primary hover:bg-vraccent-primary/10 flex items-center justify-center transition-colors"
                title="扫码识别"
              >
            ...
            <button
              ...
              disabled={orderLoading || !code.trim()}
              className="h-10 px-5 rounded-lg bg-vraccent-primary text-white text-sm font-medium hover:bg-vraccent-primary/90 transition-colors disabled:opacity-50 flex items-center gap-2"
            >
          ...
          {orderError && (
            <div className="rounded-xl p-4 bg-vrerror/10 border border-vrerror/20 text-vrerror text-sm mb-6">
              查询失败，请检查券码是否正确
            </div>
          )}

          {!order && searchCode && !orderLoading && (
            <div className="text-center py-10 text-vrtext-muted text-sm">
              未找到符合条件的团购券订单
            </div>
          )}
